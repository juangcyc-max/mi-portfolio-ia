...
import {
  type ProjectType, type Features, type BudgetRange,
  PROJECT_PRICES, FEATURE_PRICES, MONTHLY_FEE,
} from "@/lib/budgetData";
import { downloadBudgetPDF } from "@/lib/budgetPdf";

const Icons: Record<ProjectType, (props: React.SVGProps<SVGSVGElement>) => React.ReactElement> = {
  landing:    (props) => <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>,
  corporate:  (props) => <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>,
  ecommerce:  (props) => <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>,
  ia:         (props) => <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>,
  app_movil:  (props) => <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>,
  agente_voz: (props) => <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>,
};
...
export default function BudgetCalculator() {
  ...
  const [features, setFeatures] = useState<Features>({
    seo: false, chatbot: false, analytics: false,
    cms: false, multilingual: false, aiIntegration: false,
  });
  ...
  const calculateBudget = (): BudgetRange | null => {
    ...
    const base = PROJECT_PRICES[projectType];
    const extras = (Object.entries(features) as [keyof Features, boolean][])
      .reduce((sum, [f, on]) => on ? sum + FEATURE_PRICES[f] : sum, 0);
    return { min: base.min + extras, max: base.max + extras };
  };

  const budget = calculateBudget();
  const formatPrice = (n: number) => new Intl.NumberFormat("es-ES").format(n);
  ...
  const handleGetQuote = async () => {
    ...
    setSending(true);
    const featuresText = (Object.entries(features) as [keyof Features, boolean][])
      .filter(([, on]) => on)
    ...
    await fetch("/api/budget-request", {
      ...
      body: JSON.stringify({
        name: clientName,
        email: clientEmail,
        project_type: PROJECT_LABELS[projectType],
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="text-xs text-slate-400 mb-1 block font-medium">
                    {t("calc_proj_desc")} <span className="text-slate-300">{t("calc_optional")}</span>
                  </label>
                  <textarea
                    value={comment}
              ...
              {budget && showBudget ? (
                ...
                >
                  ...
                  <div className="relative z-10 text-center">
                    ...
                    {projectType && (
                      <div className="flex items-center justify-center gap-2 mb-6 px-4 py-3 rounded-2xl bg-white/5 border border-emerald-500/20">
                        ...
                        <span className="text-sm text-slate-300">
                          ...
                          <span className="font-black text-emerald-400">{formatPrice(MONTHLY_FEE[projectType])} €/mes</span>
                        </span>
                      </div>
                    )}
                    <div className="space-y-4">
                    ...
                    <p className="mt-8 text-[11px] text-slate-500 leading-relaxed italic">{t("calc_note")}</p>
                  </div>
                </motion.div>
              ) : (
                <motion.div
